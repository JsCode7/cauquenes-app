...
export async function POST() {
  ...
  try {
    if (headersList.get("x-api-key") == process.env.API_KEY) {
        ...
        const markets = response.filter((market) =>
            market.types.includes("supermarket" || "grocery_store" || "store" || "food" || "liquor_store" || "restaurant" || "home_goods_store" || "clothing_store" || "bar")
        );
